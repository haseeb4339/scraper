...
def generate_product_details(image_path, row_index):
    try:
        ...
        response = model.generate_content([
            "Generate a unique long title (dont use brand name for each title), a detailed unique and long description, and a unique 5-8 key features also for features just include numbers  for this product.",
            image
        ])
        ...
        # description_match = re.search(r"##Description:\n\n(.*?)(?=\n\n## Key Features:|$)", generated_text, re.DOTALL)
        # features_match = re.search(r"##Key Features:\n\n(.*)", generated_text, re.DOTALL)
        
        title_match = re.search(r"^##\s*(.+?)\s*(?=##)", generated_text, re.DOTALL)
        description_match = re.search(r"##\s*Description:\s*(.*?)(?=\s*##\s*Key Features:|$)", generated_text, re.DOTALL)
        features_match = re.search(r"##\s*Key Features:\s*(.*)", generated_text, re.DOTALL)
        

        # Improved parsing logic based on the updated response format
        # title_match = re.search(r"^##:\*\*\s*(.+)", generated_text, re.MULTILINE)
        # description_match = re.search(r"Description:(.*?)(?=\n\n##\s*\*\*Key Features:\*\*|$)", generated_text, re.DOTALL)
        # features_match = re.search(r"Key Features:(.*)", generated_text, re.DOTALL)
